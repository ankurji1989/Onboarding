...
import { Checkbox, DefaultButton, Dialog, DialogFooter, DialogType, Link, PrimaryButton, Spinner } from 'office-ui-fabric-react';
...
export default class TempTraining extends React.Component<ITempTrainingProps, ITempTrainingState> {
  ...
  private async getTemporaryTrainings(){
    ...
    SPDataOperations.getListItems(this.props.tempTrainingUserList,'Id,Title,StartDate,DueDate,Status,Training/Title,Training/TrainingPath,Employee/Id,Employee/EMail','Training,Employee',`Employee/EMail eq '${this.props.context.pageContext.user.email}' and DueDate ge datetime'${currentDate}'`).then((allTrainigs) => {
      this.setState({currentUser: userDetail.Title, allTraining: allTrainigs, isLoading: false, selectedTraining: []});
    });
  }
  ...
  public render(): React.ReactElement<ITempTrainingProps> {
    const {currentUser, allTraining, selectedTraining} = this.state;
    if (this.props.configured) {
    return (
      <div className={ styles.tempTraining }>
        <div className={styles.module}>
          <div className={styles.moduleHeading}><h5>{currentUser}</h5></div>
          <div className={styles.subModule}>
            ...
            <div>
              ...
              <PrimaryButton text="Save" onClick={() => this.setState({openDialogBox: true})} disabled={selectedTraining.length > 0 ? false : true}></PrimaryButton>
            ...
            :
            <div>You don't have any pending training</div>
            }
